...
;(function($) {
    ...
    $.Autocompleter = function(input, options) {
        ...
        $input.bind(($.browser.opera ? "keypress" : "keydown") + ".autocomplete", function(event) {
            ...
            //                onChange(0, true);
            //            }
            //以上是源代码,去掉了hasFocus 2010-12-7luzh
            if (!select.visible()) {
                onChange(0, true);
            }
...
//            }
    ...
    $.Autocompleter.defaults = {
        ...
        multiple: false,
        multipleSeparator: ", ",
        highlight: function(value, item) {
            //将term拆分;luzh2010-12-10
            var itemArr = $.trim(item).split(" ");
            var newArr = [];
            for (var j = 0; j < itemArr.length; j++) {
                if ($.trim(itemArr[j]) != "") {
                    newArr.push(itemArr[j]);
                }
            }
            for (var i = 0; i < newArr.length; i++) {
                value = value.replace(new RegExp("(?![^&;]+;)(?!<[^<>]*)(" + newArr[i].replace(/([\^\$\(\)\[\]\{\}\*\.\+\?\|\\])/gi, "\\$1") + ")(?![^<>]*>)(?![^&;]+;)", "gi"), "<strong>$1</strong>");
            }
            return value;
        },
        scroll: true,
            ...
            load: function(q) {
                ...
                */
                if (!options.url && options.matchContains) {
                    //luzh 在这里进行多值的匹配 2010-12-10
                    var currentArr = $.trim(q).split(" ");
                    var newArr = [];
                    for (var j = 0; j < currentArr.length; j++) {
                        if ($.trim(currentArr[j]) != "") {
                    ...
                    for (var k in data) {
                        ...
                        if (k.length > 0) {
                            ...
                            $.each(c, function(i, x) {
                                ...
                                var n = 0; //用于匹配计数
                                for (var i = 0; i < newArr.length; i++) {
                                    if (matchSubset(x.value, newArr[i])) {
                                        n++;
                                    }
                                }
                                if ((n == newArr.length) || newArr.length == 0) {
                                    csub.push(x);
                                }
                            });
                        }
